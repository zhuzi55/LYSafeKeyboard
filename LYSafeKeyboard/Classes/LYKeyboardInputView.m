...
#define SMYKeyboardCols 3
#define SMYKeyboardTextFont 20

@interface LYKeyboardInputView ()
...
@implementation LYKeyboardInputView
...
- (UIImage *)nomImage:(BOOL)isNom {
    ...
    if (isNom) {
        // 普通图片
        UIImage *image = [LYKeyboardTool getImageWithResouce:@"symbolKeyboardButtonBgNor.png"];
        return image;
    }else{
        // 高亮图片
        UIImage *highImage = [LYKeyboardTool getImageWithResouce:@"symbolKeyboardButtonBgSelect@2x.png"];
        return highImage;
...
- (void)setupTopButtonsWithImage:(UIImage *)image highImage:(UIImage *)highImage isLowercase:(BOOL)change  {
    ...
    NSMutableArray *arrM = [NSMutableArray array];
    [arrM removeAllObjects];
    if (self.letterIsOrder == YES) {
        //正序排列
        NSArray *array = [NSString DYOrderAllLetterLowercase:change];
...
- (void)setupTopSymbolButtonsWithImage:(UIImage *)image highImage:(UIImage *)highImage {
    ...
    [arrM removeAllObjects];
    NSInteger count = 0;
    if (self.symbolIsOrder == YES) {
        //正序排列
        count = [NSString DYOrderAllSymbol].count;
        arrM = [NSString DYOrderAllSymbol];
    }else{
        //乱序排列
        count = [NSString DYRandomAllSymbol].count;
...
- (void)setupBottomButtonsWithImage:(UIImage *)image highImage:(UIImage *)highImage {
    ...
    // 空格
    self.blankBtn = [self setupBottomButtonWithTitle:@"空格" image:image];
    ...
    [self.shiftButton setImage:[LYKeyboardTool getImageWithResouce:@"keyboard_littleIcon@2x.png"] forState:UIControlStateNormal];
    [self.shiftButton setImage:[LYKeyboardTool getImageWithResouce:@"keyboard_largeIcon@2x.png"] forState:UIControlStateSelected];
...
- (void)layoutSubviewst{
    {
        ...
        for (NSUInteger i = 0; i < count; i++) {
            ...
            if (i > 9 && i< 19) {
                UIButton *topButton = self.subviews[i];
                topButton.x = (self.width-topBtnW*9-colMargin*8)*0.5 + (i-10)*(topBtnW+colMargin);
                topButton.y = topMargin + topBtnH + rowMargin;
                topButton.width = topBtnW;
                topButton.height = topBtnH;
            }
            ...
            if (i > 18) {
                UIButton *buttonZero  = self.subviews[i];
                CGFloat col           = i-18;
                buttonZero.x          = leftMargin + topBtnW*0.5 + (self.width - topBtnW*7 - colMargin*6 - topBtnW*3 -colMargin*2 - leftMargin *2)*0.5 + col * (topBtnW + colMargin) ;
...
- (UIButton *)setupBottomButtonWithTitle:(NSString *)title image:(UIImage *)image {
    ...
    if (title) {
        ...
        bottomBtn.titleLabel.font = [UIFont systemFontOfSize:SMYKeyboardTextFont];
        [bottomBtn setTitleColor:[UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0] forState:UIControlStateNormal]; //颜色修改
    }
    if (image) {
        [bottomBtn setBackgroundImage:[LYKeyboardTool getImageWithResouce:@"symbolKeyboardButtonBgNor.png"] forState:UIControlStateNormal];
        [bottomBtn setBackgroundImage:[LYKeyboardTool getImageWithResouce:@"symbolKeyboardButtonBgSelect@2x.png"] forState:UIControlStateHighlighted];
